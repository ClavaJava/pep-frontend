...
export default function Internados() {
  ...
  return (
    <div>
                    ...
                    <tr key={i.id}>
                      ...
                      <td>
                        <div className="flex gap-2" style={{ flexWrap: "wrap" }}>
                          ...
                          <button className="btn btn-outline btn-sm" onClick={() => { setModalTransf(i); setActionError(null); }}>
                            🔄 Transferir
                          </button>
          ...
          {historico && (
            <div>
              ...
              {historico.internacoes.length === 0 ? (
                <div className="card"><div className="empty-state"><p>Nenhuma internação encontrada.</p></div></div>
              ) : (
        ...
        <div className="modal-overlay">
          <div className="modal" style={{ maxWidth: 420 }}>
            ...
            <div className="modal-body">
              ...
              <p className="text-muted text-sm" style={{ marginTop: 4 }}>O leito será liberado automaticamente.</p>
      ...
      {modalTransf && (
        <TransferirModal
          internacao={modalTransf}
          onClose={() => setModalTransf(null)}
          onSuccess={() => { setModalTransf(null); load(); }}
        />
      )}
...
function TransferirModal({ internacao, onClose, onSuccess }) {
  ...
  const transferir = async () => {
    if (!leitoSelecionado) { setError("Selecione um leito."); return; }
    setSaving(true); setError(null);
    try {
      await api.internacoes.transferir(internacao.id, setor);
      onSuccess();
    } catch (e) {
      setError(e.message);
    } finally {
  ...
  return (
    <div className="modal-overlay">
      <div className="modal" style={{ maxWidth: 480 }}>
        <div className="modal-header">
          <span className="modal-title">Transferir — {internacao.nomePaciente}</span>
        ...
        <div className="modal-body">
          ...
          <div className="form-group" style={{ marginBottom: 20 }}>
            ...
            <select value={setor} onChange={e => buscarLeitos(e.target.value)}>
              ...
              {SETORES.filter(s => s !== internacao.setor).map(s => (
          ...
          {setor && (
            <div className="form-group" style={{ marginBottom: 20 }}>
              ...
              {loadingLeitos ? (
                ...
                <div className="alert alert-error">Nenhum leito disponível neste setor.</div>
              ) : (
          ...
          <div className="form-actions">
            ...
            <button className="btn btn-primary" onClick={transferir} disabled={saving || !leitoSelecionado}>
              {saving ? <><span className="spinner" /> Transferindo...</> : "Confirmar Transferência"}
            </button>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal modal-lg">
        ...
        <div className="modal-body">
          <div className="tab-bar">
            <button className={`tab-btn ${tab === "prescricoes" ? "active" : ""}`} onClick={() => { setTab("prescricoes"); setSaveError(null); }}>💊 Prescrições</button>
            <button className={`tab-btn ${tab === "relatorios" ? "active" : ""}`} onClick={() => { setTab("relatorios"); setSaveError(null); }}>📝 Relatórios</button>
          </div>
            ...
            <>
              {tab === "prescricoes" && (
                <div>
                  ...
                  {prescricoes.length === 0 && <div className="empty-state" style={{ padding: "24px 0" }}><p>Nenhuma prescrição registrada.</p></div>}
                </div>
              )}
                ...
                <div>
                  ...
                  <div className="form-grid" style={{ marginBottom: 16 }}>
                    ...
                    <div className="form-group full">
                      ...
                      <textarea value={formRelatorio.descricao} onChange={e => setFormRelatorio({ ...formRelatorio, descricao: e.target.value })} placeholder="Descreva a evolução do paciente..." rows={3} />
                    </div>
                  </div>
                  ...
                  {relatorios.length === 0 && <div className="empty-state" style={{ padding: "24px 0" }}><p>Nenhum relatório registrado.</p></div>}
                </div>
              )}
            </>
          )}
